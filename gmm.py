...
if __name__=='__main__':
    ...
    x_list= ['amihud_measure_A_volume','amihud_measure_H_volume','amihud_turnover_A','amihud_turnover_H','amihud_constant_A_volume',
                'amihud_constant_H_volume',
             'AH_share_size','supply','vol'
    ]
    for key in x_list:
    ...
    regression_df['amihud_measure_AH']=regression_df['amihud_measure_A_volume']/regression_df['amihud_measure_H_volume']
    regression_df['amihud_turnover_AH']=regression_df['amihud_turnover_A']/regression_df['amihud_turnover_H']
    regression_df['amihud_constant_AH']=regression_df['amihud_constant_A_volume']/regression_df['amihud_constant_H_volume']
    del regression_df['amihud_measure_A_volume'],regression_df['amihud_measure_H_volume'],regression_df['amihud_turnover_A']
    del regression_df['amihud_turnover_H'],regression_df['amihud_constant_A_volume'],regression_df['amihud_constant_H_volume']
    ...
    regression_df = regression_df.merge(factor_df, left_index=True, right_index=True, how='left')
    '''
    factor_df=translate_commen_factor(factors_data, 'm2_growth')
    regression_df = regression_df.merge(factor_df, left_index=True, right_index=True, how='left')
    factor_df=translate_commen_factor(factors_data, 'rate_differential')
    regression_df = regression_df.merge(factor_df, left_index=True, right_index=True, how='left')
    factor_df=translate_commen_factor(factors_data, 'export_import')
    regression_df = regression_df.merge(factor_df, left_index=True, right_index=True, how='left')
    factor_df=translate_commen_factor(factors_data, 'exch')
    regression_df = regression_df.merge(factor_df, left_index=True, right_index=True, how='left')
    '''
    ...
    # #Test is AHGMM works correctly
    # x_list=['constant','amihud_measure_AH','amihud_turnover_AH', 'amihud_constant_AH','AH_share_size', 'supply', 'vol']
    # instruments_list=x_list
    # y_list=['discount_y']
    # x=regression_df_lag[x_list].values
    # y=regression_df_lag[y_list].values
    # instruments=regression_df_lag[instruments_list].values
    # #OLS
    # model_ols=sm.OLS(y,x)
    # res = model_ols.fit()
    # print(res.summary())
    # #GMM
    # model_gmm=AHGMM(y,x,instruments,instruments_list)
    # model_gmm.fitgmm()
    # model_gmm.summary()
    ...
    generate_instruments_list=[
        'discount_y',
        'amihud_measure_AH','amihud_turnover_AH', 'amihud_constant_AH',
        'AH_share_size', 'supply', 'vol',
        'csi_hsc',
        #'m2_growth','rate_differential','export_import','exch'
        ]
    ...
    x_regression_list = ['constant',
                         ...
                         #'AH_share_size', 'supply', 'vol'
                         #'csi_hsc',
                         #'m2_growth','exch','rate_differential','export_import'
                         ]
    y_regression_list=['discount_y']
    instruments_list=['amihud_measure_AH',
                      'amihud_turnover_AH',
                      'amihud_constant_AH',
                      'amihud_measure_AH_diff',
                      'amihud_turnover_AH_diff',
                      'amihud_constant_AH_diff',
                      #'AH_share_size', 'supply', 'vol',
                      #'csi_hsc',
                      'csi_hsc_diff',
                      'AH_share_size_diff', 'supply_diff', 'vol_diff',
                      #'m2_growth', 'exch', 'rate_differential', 'export_import',
                      #'m2_growth_diff', 'exch_diff', 'rate_differential_diff', 'export_import_diff',
                      #'discount_y_diff'
                      ]

    '''
    'amihud_measure_AH','amihud_turnover_AH', 'amihud_constant_AH',
    'amihud_measure_AH_diff','amihud_turnover_AH_diff', 'amihud_constant_AH_diff',
    'AH_share_size', 'supply', 'vol',
    'AH_share_size_diff', 'supply_diff', 'vol_diff'    
    '''
